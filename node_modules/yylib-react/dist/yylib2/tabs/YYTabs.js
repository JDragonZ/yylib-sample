...
var YYTabs = YYClass.create({
    ...
    contextMenuHandler: function contextMenuHandler(event) {
        ...
        if (tabs.length > 0) {
            tabs.forEach(function (item) {
                if (item.props.tab === target.textContent) {
                    key = item.key;
                    closeable = item.props.closeable ? true : false;
                }
            });
            //统计可关闭标签页数量
            tabs.forEach(function (item) {
                if (item.props.closeable) {
                    ++closeableNum;
                }
            });
        }
    ...
    render: function render() {
        ...
        var childrenCloseable;
        if (type === 'page') {
            childrenCloseable = [];
            React.Children.forEach(children, function (child, index) {
                childrenCloseable.push(React.cloneElement(child, {
                    tab: React.createElement(
                        'div',
                        null,
                        child.props.tab,
                        child.props.closeable && editable ? React.createElement(YYIcon, { type: 'cross', onClick: function onClick(e) {
